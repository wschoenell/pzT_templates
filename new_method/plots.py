...
def make_hist(xmin, xmax, dx, parameter, weights, axis, prior=None, bins=None):
    if bins is None:
...
file_fit = h5py.File('%s/bpz_fit_magerr_%3.2f.hdf5' % (save_dir, config['magerr']), 'r')
...
model_magnitudes = np.load('model_magnitudes.npz')['arr_0']  # FIXME: file_fit.get('/model_magnitudes')
...
templates = np.array(range(n_templates))  # 0, n_templates, config['n_interpolations']))
...
if not paper:
    for i_template in templates:
        ...
        # M/L ratio histogram
        make_hist(0.4, 4., 0.2, parameters['m2l'][i_z, i_template], likelihood[i_z, i_template], axes[2])
        axes[2].set_title('$\log M / L (\lambda = 4020 \\AA)$')
        axes[2].xaxis.get_ticklabels()[-1].set_visible(False)

        # Metallicity ratio histogram
        make_hist(-4.0001, -1.31, .2, np.log10(parameters['metallicity'][i_z, i_template]), likelihood[i_z, i_template],
                  axes[3], bins=np.log10(np.unique(np.sort(parameters['metallicity']))))
        ...
        axes[-1].plot(filter_lambdas, bpz_template_magnitudes[i_z, i_template], color='red', lw=2, alpha=0.8)
        axes[-1].errorbar(filter_lambdas, bpz_template_magnitudes[i_z, i_template], color='red',
                          yerr=0.05)
        # Plot filters used for colors on template_colorspace
        for id in np.ravel(plot_fid):
            axes[-1].scatter(filter_lambdas[id], bpz_template_magnitudes[i_z, i_template][id], c='yellow', s=100,
...
if paper:
    ...
    for i_template in templates:
        ...
        i_ax += 1
        # Metallicity ratio histogram
        make_hist(-4, -1.31, .2, np.log10(parameters['metallicity'][i_z, i_template]), likelihood[i_z, i_template],
                  axes[i_ax])
        axes[i_ax].xaxis.get_ticklabels()[-1].set_visible(False)
        axes[i_ax].yaxis.set_ticklabels([])
